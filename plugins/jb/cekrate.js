...
const handler = async (m, {conn}) => {
  try {
    ...
    if (!data || !data.rate) {
      return conn.sendMessage(m.chat, { text: "belum ada rate yang di set di chat ini, gunakan .setrate 100-150" }, { quoted: m })
    }
    return conn.sendMessage(m.chat, { text: `rate saat ini: *${data.rate}*` }, { quoted: m })
  } catch (err) {
